...
Q1
From the column mixed_value, extract the numeric characters that appear at the very beginning of
the value.
The extraction must stop as soon as a non-numeric character appears.

Q2
From the column mixed_value, extract the numeric characters that appear at the very end of the
value.
...
Q5
From the column mixed_value, extract exactly two consecutive numeric characters that appear
...
Q8
From the column mixed_value, extract the numeric portion that is present between alphabetic
characters.
...
Q10
From the column email, extract the text that appears after the at-symbol including the domain
name.
...
Q13
From the column mixed_value, extract only alphabetic characters that appear together as a
continuous sequence.
...
Q17
From the column full_text, extract the employee number portion that appears between the
...
Q19
From the column full_text, extract the alphabetic text that appears between two underscore
characters.
